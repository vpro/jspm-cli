...
if (systemVersion.match(/\/systemjs\#/))
...
exports.dlLoader = function(unminified, edge, latest) {
  ...
  .then(function() {
    return dl('systemjs', 'github:vpro/systemjs', !edge ? (!latest ? systemVersion : '^' + systemVersion) : 'master')
...
exports.init = function init(basePath) {
  ...
  .then(function() {
    ui.log('info', '');
    ui.log('ok', 'package.json at %' + path.relative(process.cwd(), config.pjsonPath) + '%\n' +
